...
# 
  ...
  def full_filename(thumbnail = nil)
    ...
    case self.thumbnail
    when "thumb"
      File.join(Rails.root, file_system_path, 'thumb', thumbnail_name_for(thumbnail, self.parent_id))
    else
      File.join(Rails.root, file_system_path, 'fullsize', thumbnail_name_for(thumbnail_name_for, self.id))
    end
  end
